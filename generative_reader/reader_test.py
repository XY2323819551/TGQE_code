...
def evaluate(model, dataset, dataloader, tokenizer, opt):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            # orig原始逻辑，用的时候要绑定修改model.py中line 54 左右的两个参数嗷
            ...
            # 这里适用于bs=1, num_return_sequences=5, num_beams=6的时候
            # 用的时候要绑定修改model.py中line 54 左右的两个参数嗷
            # example = dataset.data[idx[0]]
            # ans = ""
            # score = False
            # for o in outputs:
            #     sub_ans = tokenizer.decode(o, skip_special_tokens=True)
            #     ans += sub_ans + "\t"
            #     sub_score = src.evaluation.ems(ans, example['answers'])
            #     if sub_score:
            #         score = True
            # exactmatch.append(score)
            # if opt.write_results:
