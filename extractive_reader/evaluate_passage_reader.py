...
from .base import Reader, Question, Context
from .dpr_reader import DprReader
from .span_selection import DprSelection, GarSelection, DprFusionSelection, GarFusionSelection
from .retrieval import RetrievalExample
from .evaluate import ReaderEvaluator
...
def main():
    ...
    for _, item in data.items():
        ...
        #                 : max_topk_passages]
        topk_contexts = item['contexts'][:max_topk_passages]  # 如果是拿rider,rerank后的文件进行read，那么就不需要排序，这个直接截取就可以
    ...
    rider_predictions = {} if args.output_file is not None else None

    ems = evaluator.evaluate(examples, options.topk_em, rider_predictions)
    ...
    if args.output_file is not None:
        with open(args.output_file, 'w', encoding='utf-8', newline='\n') as f:
            json.dump(rider_predictions, f, indent=4)
