...
def setup_task():
    # set target
    dataset = 'trivia'  # self-defined，切换数据集
    # dataset = 'nq'
    mode = 'multi-inputs'
    ...
    if data_path is None:
        ...
        if os.path.exists(mount_path):
            ...
            data_path = '../data/gar_generation_data_trivia/'  # self-defined
            output_path = '../data/output_model_trivia/'  # self-defined
    ...
    if 'multi-inputs' in mode:
        ...
        bs_eval = 16
    if dataset == 'nq':
        # MAX_SRC_LENGTH = 20  # 99%, 原设置
        MAX_SRC_LENGTH = 768  # self-defined
        if mode == 'title':
        ...
        # MAX_SRC_LENGTH = 64  # 95%~99%，ori 50
        MAX_SRC_LENGTH = 768  # self-defined
        if mode == 'title':
            ckpt_name = Path(data_path) / "checkpointepoch=XXX.ckpt"
            data_path = Path(data_path) / 'trivia-title/'
            MAX_TGT_LENGTH = 64  # 80%  self-defined
        elif mode == 'answer':
            ckpt_name = Path(data_path) / "checkpointepoch=XXX.ckpt"
            data_path = Path(data_path) / 'trivia-answer'
            MAX_TGT_LENGTH = 48  # 99%  self-defined
        elif mode == 'sentence':
            ckpt_name = Path(data_path) / "checkpointepoch=XXX.ckpt"
            data_path = Path(data_path) / 'trivia-sentence/'
            MAX_TGT_LENGTH = 128  # 80% ~ 95%  self-defined
        elif mode == 'multi-inputs':
            MAX_SRC_LENGTH = 1024
