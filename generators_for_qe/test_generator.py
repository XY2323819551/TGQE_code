...
def generate_summaries(
    ...
    for batch in tqdm(list(chunks(examples, batch_size))):
        ...
        sampling = True  # self-defined, default False
        if sampling:
            dataset = 'nq'
            if dataset == 'nq':
                temperature = 5
                ...
                temperature = 2
                top_p = .5
            summaries = model.generate(num_beams=5,  # self-defined, default 1
                                       max_length=max_tgt_len,
                                       do_sample=True,
                                       temperature=temperature,
                                       top_p=top_p,
                                       num_return_sequences=5,  # self-defined, default 10
                                       early_stopping=True, **dct)
        else:
            summaries = model.generate(num_beams=5,  # self-defined, default num_beams=1
                                       max_length=max_tgt_len,
                                       early_stopping=True, **dct)
